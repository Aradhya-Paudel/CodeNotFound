...
function GuestUser() {
  ...
  const submitCapture = async () => {
    ...
    try {
      ...
      // Save to submissions.json via API
      // Actually, we send to /incidents to create a real alert
      if (true) { // Always try API based on current setup
        try {
          await api.post('/incidents', {
            title: "Guest Report (AI Pending)", // Backend will update title if needed
            image: capturedImage, // Base64 string
            location: location,
            description: "Reported via Guest Portal. Analyzing..."
          });
          console.log("Incident submitted successfully via API");
        } catch (apiError) {
