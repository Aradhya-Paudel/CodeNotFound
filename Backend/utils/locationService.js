...
const BASE_URL_DIRECTIONS = 'https://us1.locationiq.com/v1/directions/driving';
const BASE_URL_MATRIX = 'https://us1.locationiq.com/v1/matrix/driving';
...
/**
 * Reverse geocodes coordinates to a text address.
 */
...
/**
 * Gets the driving route between two points. 
 * Returns GeoJSON shape for map display and duration/distance.
 */
const getRoute = async (startLat, startLon, endLat, endLon) => {
    if (!API_KEY) return null;
    try {
        const coords = `${startLon},${startLat};${endLon},${endLat}`;
        const response = await axios.get(`${BASE_URL_DIRECTIONS}/${coords}`, {
            params: {
                key: API_KEY,
                overview: 'full',
                geometries: 'geojson',
        ...
        console.error('LocationIQ Routing Error:', error.message);
        return null;
    }
};

/**
 * Gets travel times between one source and multiple destinations.
 * Useful for finding the 'Quickest' hospital, not just nearest.
 */
const getTravelTimes = async (originLat, originLon, destinations) => {
    // destinations: Array of {lat, lon, id}
    if (!API_KEY || !destinations.length) return null;

    try {
        // Format: srcLon,srcLat;dstLon1,dstLat1;dstLon2,dstLat2...
        const coords = [`${originLon},${originLat}`, ...destinations.map(d => `${d.lon},${d.lat}`)].join(';');

        const response = await axios.get(`${BASE_URL_MATRIX}/${coords}`, {
            params: {
                key: API_KEY,
                sources: 0, // 0 is the origin index
                annotations: 'duration'
            }
        });

        if (response.data && response.data.durations && response.data.durations.length > 0) {
            // Map back to destination IDs
            // Response durations[0] is array of travel times from source 0 to all points
            const distinctTimes = response.data.durations[0].slice(1); // skip 0->0

            return destinations.map((d, i) => ({
                id: d.id,
                duration: distinctTimes[i] // seconds
            }));
        }
        return null;

    } catch (error) {
        console.error('LocationIQ Matrix Error:', error.message);
        return null;
    }
};

// ... Legacy Map Generators (Keep for backward compatibility)
