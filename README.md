...
Bachoo is an emergency response platform that connects ambulances with the right hospitals in real-time. When someone needs emergency care, our system makes sure they get to a hospital that actually has what they need - the right blood type, the right specialists, and available beds - all on the first try.
...
```
Emergency Call → Ambulance Dispatch → Patient Assessment → System Input
                                                                  ↓
                                                    Matching Algorithm
...
We're a group of students passionate about using technology to solve real problems. For this hackathon, we divided up the work - handling documentation and presentation, building the frontend, developing the backend, and designing the database and APIs.
